...
#define _EMA_ALPHA 0.00000001   // EMA weight of new sample (range: 0 to 1)
...
void loop() {
  ...
  if ((dist_raw-dist_prev>1.0)&&(dist_prev-dist_raw>=1.0))
    dist_raw=dist_prev;
  ...
  if (dist_ema<=180.0){
    ...
    float proportion = (dist_ema - 180.0) / (360.0 - 180.0);
    int servo_angle = _DUTY_MIN + proportion * (_DUTY_MAX - _DUTY_MIN);
